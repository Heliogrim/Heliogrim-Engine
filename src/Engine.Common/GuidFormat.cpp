...
Guid hg::decodeGuid4228(string_view str_) {
    ...
    tmp.pre = _STD stoul(string { str_.substr(0, c0Idx - 0 - 1) });
    tmp.c0 = _STD stoi(string { str_.substr(c0Idx, c1Idx - c0Idx - 1) });
    tmp.c1 = _STD stoi(string { str_.substr(c1Idx, postIdx - c1Idx - 1) });
    tmp.post = _STD stoull(string { str_.substr(postIdx, string_view::npos) });
