...
smr<TextureResource> TextureTransformer::transpose(
	...
	auto view = _pool->allocateSparseTexture(
		pool::SparseTextureAllocation {
			...
			.mipLevels = { 0uL, asset->getMipLevelCount() - u32 { 1u } },
	...
	transformer::convertKtx(asset, std::move(from_), view.get(), _pool->device(), options_);
...
void TextureTransformer::partialTranspose(
	...
	transformer::convertKtxPartial(asset, std::move(from_), view, _pool->device(), options_);
}
