...
namespace hg::engine::scene {
	...
	constexpr ptr<Ty> construct_inplace(ptr<void> dst_, Alloc_ allocator_ = Alloc_ {}) {
	 ...
	 */
	template <typename Ty, typename Alloc_ = std::allocator<Ty>>
	constexpr ptr<Ty> construct_inplace(ptr<void> dst_, cref<Ty> src_, Alloc_ allocator_ = Alloc_ {}) {
		using allocator_trait = std::allocator_traits<Alloc_>;
	 ...
	 */
	template <typename Ty, typename Alloc_ = std::allocator<Ty>>
	constexpr ptr<Ty> construct_inplace(ptr<void> dst_, mref<Ty> src_, Alloc_ allocator_ = Alloc_ {}) {
		using allocator_trait = std::allocator_traits<Alloc_>;
	 ...
	 */
	template <typename Ty, typename Alloc_ = std::allocator<Ty>>
	constexpr void destruct_inplace(
		ptr<Ty> dst_,
	...
	private:
		...
		constexpr static page_index_type unmask_page_index(cref<index_type> masked_) noexcept {
			return static_cast<page_index_type>((masked_ & index_page_mask) >> index_page_shift);
		}

		constexpr static value_index_type unmask_value_index(cref<index_type> masked_) noexcept {
			return static_cast<value_index_type>(masked_ & index_value_mask);
		}

		constexpr static void unmask(
