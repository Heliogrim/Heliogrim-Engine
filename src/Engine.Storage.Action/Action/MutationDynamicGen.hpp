...
namespace hg::engine::storage::system {
	class MutationDynamicGen final :
		public Mutation {
	public:
		using this_type = MutationDynamicGen;

		using index_type = std::size_t;

	public:
		MutationDynamicGen() = delete;

		explicit MutationDynamicGen(_Inout_ mref<InlineAutoArray<DynamicStage, 3>> stages_) noexcept;

		MutationDynamicGen(mref<this_type>) noexcept = default;

		~MutationDynamicGen() noexcept override = default;
