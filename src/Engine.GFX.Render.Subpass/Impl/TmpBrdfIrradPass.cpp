...
void TmpBrdfIrradPass::execute(cref<graph::ScopedSymbolContext> symCtx_) noexcept {
	...
	if (_brdfIrrad.empty()) {
		...
		auto brdfIrrad = factory->build(
			{
				...
				MAX(static_cast<u32>(math::floorf(math::log2f(64.F))) + 1uL, 1uL),
				TextureType::eCube,
		...
		_brdfIrrad = make_smr<Texture>(std::move(brdfIrrad));
	}
...
smr<AccelerationEffect> build_test_effect() {
	...
	const auto vertexShaderCode = read_shader_file(R"(brdf/__test__brdfIrrad.gen.vs)");
	...
	const auto fragmentShaderCode = read_shader_file(R"(brdf/__test__brdfIrrad.gen.fs)");
