...
void TmpDirectionalShadowPass::execute(cref<graph::ScopedSymbolContext> symCtx_) noexcept {
...

	{
		...
		for (auto i = 0u; i < cascadeCount; ++i) {
			...
			const auto log = minZ * math::powf(ratio, p);
		...
		for (auto c = 0u; c < cascadeCount; ++c) {
			...
			radius = math::ceilf(radius * 16.F) / 16.F;
			...
			//const auto lightDirection = (-origin.into()).normalized();
			...
			//const auto castOrigin = origin.into() + lightDirection * (-1.F * -1.F) * distance;
