...
namespace hg::engine::resource {
	template </*typename Derived_, */typename FinalFn_, typename... StageFn_>
	class LoadPipeline :
		protected FinalFn_,
		protected StageFn_... {
	public:
		using this_type = LoadPipeline</*Derived_, */FinalFn_, StageFn_...>;
		//using derived_type = Derived_;

	public:
		explicit LoadPipeline(FinalFn_&& finalStage_, StageFn_&&... stages_) :
			FinalFn_(std::forward<FinalFn_>(finalStage_)),
			StageFn_(std::forward<StageFn_>(stages_))... {}

		constexpr virtual ~LoadPipeline() noexcept = default;
	...
	LoadPipeline(StageFn_&&... stages_) -> LoadPipeline<StageFn_...>;
}
