...
namespace hg::engine::reflow {
	...
	struct ExtractTypeOfImpl<NamedAttribute<Name_, LayoutAttribute<Attribute_>>> {
		using named_type = NamedAttribute<Name_, LayoutAttribute<Attribute_>>;
	...
	class LayoutAttributes {
		using this_type = LayoutAttributes<Attributes_...>;
	...
	public:
		constexpr LayoutAttributes() = default;

		constexpr explicit LayoutAttributes(auto&&... args_)
			requires (sizeof...(args_) == sizeof...(Attributes_)) :
			_attributes(::std::make_tuple<Attributes_...>(::hg::forward<decltype(args_)>(args_)...)) {}

		constexpr LayoutAttributes(cref<this_type> other_) noexcept = default;

		constexpr LayoutAttributes(mref<this_type> other_) noexcept = default;
