...
namespace hg::engine::reflow {
	...
	public:
		using this_type = StyleAttribute<Type_>;
		using store_type = meta::remove_reference_t<Type_>;

	public:
		StyleAttribute() = default;

		constexpr explicit StyleAttribute(auto&& first_, auto&&... rest_)
		...
		constexpr StyleAttribute(cref<this_type> other_) noexcept :
			_store(other_._store) {}

		constexpr StyleAttribute(mref<this_type> other_) noexcept :
			_store(::hg::move(other_._store)) {}

		constexpr ~StyleAttribute() = default;
