...
namespace hg {
    ...
    concept has_into_typename_member_function = requires(SourceType_& obj_) {
        { obj_.template into<TargetType_>() } -> std::same_as<TargetType_>;
    };
    ...
    concept has_into_wrap_template_member_function = requires(SourceType_& obj_) {
        { obj_.template into<TargetType_>() } -> std::same_as<TemplateType_<TargetType_>>;
    };
    ...
    template <
        ...
        template <typename...> typename TemplateType_ = std::type_identity_t
    >
